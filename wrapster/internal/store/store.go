...
func (c *Cache) Summary(ctx context.Context, now time.Time) (Summary, error) {
	var summary Summary
...
`, now.Unix(), now.Unix()).Scan(
	...
	if oldest.Valid {
		summary.OldestUnix = oldest.Int64
	}
	if newest.Valid {
		summary.NewestUnix = newest.Int64
	}
	return summary, nil
}
